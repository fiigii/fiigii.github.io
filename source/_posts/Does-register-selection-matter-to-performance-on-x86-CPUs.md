...
Programmers often assume that using 256-bit YMM registers should be 2x faster than 128-bit XMM registers. Actually, that is not always true. Windows x64 calling conventions define [XMM0-XMM15 as callee-saved registers](https://docs.microsoft.com/en-us/cpp/build/x64-calling-convention?view=vs-2019#callercallee-saved-registers), so using YMM0-YMM15 would require more caller-saving code than XMM registers. Moreover, Intel only implemented store forwarding for registers <= 128 bits, so spilling YMM registers can be more expensive than XMM. These additional overheads can reduce the benefits of using YMM.
